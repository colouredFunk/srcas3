package {
	...
	final public class PWRD {
		public static var autoPopup:Boolean = true;
		public static var callBack:Function;
		public static var callBackParams:Array;
		private static var isRequesting:Boolean;
		private static var onLoadedHandlerLast:Function;
		private static var tempParams:Array;
		...
		private static const RESULT_UNLOGIN:String = "0";
		private static const RESULT_LOGIN:String = "1";
		...
		private static function sendAndLoad(_url:String, _data:* = null, _method:String = "GET", _loaded:Function = null):void {
			...
			if (_loaded != null){
				...
				onLoadedHandlerLast = _loaded;
			}
			urlLoader.addEventListener(IOErrorEvent.IO_ERROR, onRequestErrorHandler);
			urlLoader.load(urlRequest);
		}

		public static function isLogin(_sessionORXML:* = "USER", _path:String = "/extend/", _otherParams:Object = null):void {
			if (isRequesting){
				return;
			}
			isRequesting = true;
			tempParams = [_sessionORXML, _path, _otherParams];
			if (_sessionORXML is XML){
				_path = String(_sessionORXML.@path) || _path;
				_sessionORXML = String(_sessionORXML.@session) || "USER";
			}
			var _url:String = _path + URL_ISLOGIN;
			if (!_otherParams){
				_otherParams = {};
			}
			_otherParams.sessionName = _sessionORXML;
			_otherParams.rand = Math.random();
			sendAndLoad(_url, _otherParams, "GET", onIsLoginRequestCompleteHandler);
		}

		public static function loginWebMode(_sessionORXML:* = "USER", _path:String = "/extend/", _otherParams:Object = null):* {
			if (ExternalInterface.available){
				...
				_otherParams.session = _sessionORXML;
		...
		private static function onRequestErrorHandler(_evt:IOErrorEvent):void {
			...
			urlLoader.removeEventListener(Event.COMPLETE, onLoadedHandlerLast);
			isRequesting = false;
			onIsLoginRequestCompleteHandler(_evt);
		}
		...
		private static function onIsLoginRequestCompleteHandler(_evt:Event):void {
			...
			var _str:String;
			if (urlLoader.data) {
				_str = String(urlLoader.data).replace(/\D/g, "");
			}else {
				_str = "error";
			}
			var _isLogin:Boolean = _str == RESULT_LOGIN;
			if (autoPopup) {
				if (_isLogin || (Capabilities.playerType != "ActiveX" && Capabilities.playerType != "PlugIn")) {
					if (callBack != null) {
						if (callBackParams) {
							callBack.apply(PWRD, callBackParams);
						}else {
							callBack();
						}
					}
				} else {
					loginWebMode.apply(PWRD, tempParams);
				}
			} else {
				if (callBack != null) {
					if (callBackParams) {
						callBack.apply(PWRD, callBackParams.unshift(_isLogin));
					}else {
						callBack(_isLogin);
					}
				}
			...
			callBack = null;
			destroyObject(callBackParams);
			callBackParams = null;
			destroyObject(tempParams);
			tempParams = null;
		}
