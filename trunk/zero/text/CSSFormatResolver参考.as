		...
		static private function addStyleAttributes(attr:TextLayoutFormatValueHolder, styleSelector:String):TextLayoutFormatValueHolder
	 	{
			...
			//var foundStyle:CSSStyleDeclaration = StyleManagerImpl.getInstance().getStyleDeclaration(styleSelector);
 		...
 		public function resolveUserFormat(elem:Object,userStyle:String):*
 		{
 			...
 			if (flowElem)
 			{
 				if (flowElem.id)
 				{
					...
					//cssStyle = StyleManagerImpl.getInstance().getStyleDeclaration("#"+flowElem.id);
 				...
 				if (flowElem.styleName)
 				{
					...
					//cssStyle = StyleManagerImpl.getInstance().getStyleDeclaration("."+flowElem.styleName);
				...
				//cssStyle = StyleManagerImpl.getInstance().getStyleDeclaration(dictionaryName == null ? elemClassName : dictionaryName);
 				if (cssStyle)
 				{
 					propStyle = cssStyle.getStyle(userStyle);
 					if (propStyle !== undefined)
