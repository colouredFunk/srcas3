...
package zero.swf.avm1{
	...
	public class ACTIONRECORD/*{*/implements I_zero_swf_CheckCodesRight{
			...
			while(offset<endOffset){
				...
				if(op<0x80){
					...
					switch(op){
						...
						break;
						case AVM1Ops.setRegister:
							code=new AVM1Code(op,data[offset++]);
						break;
						case AVM1Ops.constants:
		...
		public function toData(_toDataOptions:zero_swf_ToDataOptions):ByteArray{
			...
			if(constCode){
				...
				constCode=new AVM1Code(AVM1Ops.constants,[]);
			...
			for each(code in codeArr){
				...
				if(code is AVM1LabelMark){
					...
					if(code is int){
						...
						}else if(code.op==AVM1Ops.constants){
							if(constCode){
								if(constCode==code){
								}else{
		...
		public function toXML(xmlName:String,_toXMLOptions:zero_swf_ToXMLOptions):XML{
			...
			if(codeArr.length){
				...
				return new XML("<"+xmlName+" class=\"ACTIONRECORD\"><![CDATA[\n"+
					codesStr
					+"\t\t\t\t]]></"+xmlName+">");
			}
			return <{xmlName}/>;
