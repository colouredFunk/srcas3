...
package zero.swf.tagBodys{
	...
	public class PlaceObject3/*{*/implements I_zero_swf_CheckCodesRight{
		...
		public var SurfaceFilterV:Vector.<Object>;
		...
		public function initByData(data:ByteArray,offset:int,endOffset:int,_initByDataOptions:zero_swf_InitByDataOptions):int{
			...
			if(PlaceFlagHasFilterList){
				...
				SurfaceFilterV=new Vector.<Object>(NumberOfFilters);
				for(var i:int=0;i<NumberOfFilters;i++){
			
					SurfaceFilterV[i]=new FilterTypes.classV[data[offset++]]();
		...
		public function toData(_toDataOptions:zero_swf_ToDataOptions):ByteArray{
			...
			if(PlaceFlagHasFilterList){
				...
				for each(var SurfaceFilter:Object in SurfaceFilterV){
					data[offset++]=FilterTypes[getQualifiedClassName(SurfaceFilter).split("::").pop()];
					data.position=offset;
		...
		public function toXML(xmlName:String,_toXMLOptions:zero_swf_ToXMLOptions):XML{
			...
			if(PlaceFlagHasFilterList){
				if(SurfaceFilterV.length){
					...
					for each(var SurfaceFilter:Object in SurfaceFilterV){
						listXML.appendChild(SurfaceFilter.toXML("SurfaceFilter",_toXMLOptions));
					}
		...
		public function initByXML(xml:XML,_initByXMLOptions:zero_swf_InitByXMLOptions):void{
			...
			if(PlaceFlagHasFilterList){
				if(xml.SurfaceFilterList.length()){
					...
					var i:int=-1;
					SurfaceFilterV=new Vector.<Object>(SurfaceFilterXMLList.length());
					for each(var SurfaceFilterXML:XML in SurfaceFilterXMLList){
						i++;
						SurfaceFilterV[i]=new FilterTypes.classV[FilterTypes[SurfaceFilterXML["@class"].toString()]]();
						SurfaceFilterV[i].initByXML(SurfaceFilterXML,_initByXMLOptions);
					}
				}else{
					SurfaceFilterV=new Vector.<Object>();
				}
			}
			if(PlaceFlagHasBlendMode){
			...
			if(PlaceFlagHasClipActions){
				ClipActions=new CLIPACTIONS();
				ClipActions.initByXML(xml.ClipActions[0],_initByXMLOptions);
			}
		}
