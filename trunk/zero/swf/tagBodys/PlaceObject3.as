...
//PlaceFlagHasOpaqueBackground		UB[1]																					//20111021，属性面板里的“位图缓存 / 不透明”
//PlaceFlagHasHidden				UB[1]																					//20111021，属性面板里的勾选“是否可见”
...
package zero.swf.tagBodys{
	...
	public class PlaceObject3{
		...
		public var PlaceFlagHasOpaqueBackground:Boolean;//01000000
		...
		public var Hidden:int;//UI8
		public var OpaqueBackground:uint;//RGBA
		...
		public function initByData(data:ByteArray,offset:int,endOffset:int,_initByDataOptions:Object):int{
			...
			PlaceFlagHasOpaqueBackground=((flags&0x40)?true:false);//01000000
			var PlaceFlagHasHidden:Boolean=((flags&0x20)?true:false);//00100000
			...
			//if(PlaceFlagHasHidden){
			//	Value0x20=data[offset++];
			//}
			...
			if(PlaceFlagHasOpaqueBackground){
				OpaqueBackground=(data[offset++]<<16)|(data[offset++]<<8)|data[offset++]|(data[offset++]<<24);
			}
...
//				data.writeByte(0x00);
			...
			if(PlaceFlagHasOpaqueBackground){
				data[data.length]=OpaqueBackground>>16;
				data[data.length]=OpaqueBackground>>8;
				data[data.length]=OpaqueBackground;
				data[data.length]=OpaqueBackground>>24;
			}
		...
		CONFIG::USE_XML{
			...
			public function toXML(xmlName:String,_toXMLOptions:Object):XML{
				...
				if(PlaceFlagHasOpaqueBackground){
					xml.@OpaqueBackground="0x"+BytesAndStr16._16V[(OpaqueBackground>>24)&0xff]+BytesAndStr16._16V[(OpaqueBackground>>16)&0xff]+BytesAndStr16._16V[(OpaqueBackground>>8)&0xff]+BytesAndStr16._16V[OpaqueBackground&0xff];
				}
			...
			public function initByXML(xml:XML,_initByXMLOptions:Object):void{
				...
				var HiddenXML:XML=xml.@Hidden[0];
				if(HiddenXML){
					Hidden=int(HiddenXML.toString());
				}else{
					Hidden=-1;
				}
				
				var OpaqueBackgroundXML:XML=xml.@OpaqueBackground[0];
				if(OpaqueBackgroundXML){
					PlaceFlagHasOpaqueBackground=true;
					OpaqueBackground=uint(OpaqueBackgroundXML.toString());
				}else{
					PlaceFlagHasOpaqueBackground=false;
				}
