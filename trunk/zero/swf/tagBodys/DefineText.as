...
package zero.swf.tagBodys{
	...
	public class DefineText{//implements I_zero_swf_CheckCodesRight{
		...
		public function initByData(data:ByteArray,offset:int,endOffset:int,_initByDataOptions:Object/*zero_swf_InitByDataOptions*/):int{
			...
			_initByDataOptions.TextColorUseRGBA=false;//20110817
			...
			//trace("_initByDataOptions.GlyphBits="+_initByDataOptions.GlyphBits);
			//trace("_initByDataOptions.AdvanceBits="+_initByDataOptions.AdvanceBits);
			
			var TextRecordClass:Class;
			if(_initByDataOptions){
				if(_initByDataOptions.classes){
					TextRecordClass=_initByDataOptions.classes["zero.swf.records.texts.TEXTRECORD"];
				}
			}
			if(TextRecordClass){
			}else{
				TextRecordClass=TEXTRECORD;
			}
			var i:int=-1;
			TextRecordV=new Vector.<*>();
			while(data[offset]){//EndOfRecordsFlag
				i++;
				TextRecordV[i]=new TextRecordClass();
		...
		public function toData(_toDataOptions:Object/*zero_swf_ToDataOptions*/):ByteArray{
			...
			_toDataOptions.TextColorUseRGBA=false;//20110817
			...
			//trace("_toDataOptions.GlyphBits="+_toDataOptions.GlyphBits);
			//trace("_toDataOptions.AdvanceBits="+_toDataOptions.AdvanceBits);
			...
			data[data.length]=0;//EndOfRecordsFlag
			return data;
		}
		...
		public function toXML(xmlName:String,_toXMLOptions:Object/*zero_swf_ToXMLOptions*/):XML{
			...
			_toXMLOptions.TextColorUseRGBA=false;//20110817
			
			var xml:XML=<{xmlName} class="zero.swf.tagBodys.DefineText"
				id={id}
			/>;
			...
			if(TextRecordV.length){
				...
				for each(var TextRecord:* in TextRecordV){
		...
		public function initByXML(xml:XML,_initByXMLOptions:Object/*zero_swf_InitByXMLOptions*/):void{
			...
			for each(var TextRecordXML:XML in xml.TextRecordList.TextRecord){
				i++;
				TextRecordV[i]=new (_initByXMLOptions&&_initByXMLOptions.customClasses&&_initByXMLOptions.customClasses[TextRecordXML["@class"].toString()]||TEXTRECORD)();
