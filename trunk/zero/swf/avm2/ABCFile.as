...
package zero.swf.avm2{
	...
	public class ABCFile{//implements I_zero_swf_CheckCodesRight{
		...
		public function toXML(xmlName:String,_toXMLOptions:Object/*zero_swf_ToXMLOptions*/):XML{
			...
			_toXMLOptions.stringV=stringV;
			_toXMLOptions.namespace_infoV=namespace_infoV;
			_toXMLOptions.ns_set_infoV=ns_set_infoV;
			_toXMLOptions.multiname_infoV=multiname_infoV;
			...
			var trait:Traits_info,instance_info:Instance_info,class_info:Class_info,script_info:Script_info;
			var methodNameArr:Array=new Array();
			for each(instance_info in instance_infoV){
				for each(trait in instance_info.itraits_infoV){
					switch(trait.kind_trait_type){
						...
						case TraitTypeAndAttributes.Setter:
							methodNameArr[trait.u30_2]=multiname_infoV[trait.name];
						break;
					}
				}
			}
			for each(class_info in class_infoV){
				for each(trait in class_info.ctraits_infoV){
					switch(trait.kind_trait_type){
						case TraitTypeAndAttributes.Method:
						case TraitTypeAndAttributes.Getter:
						case TraitTypeAndAttributes.Setter:
			...
			for each(script_info in script_infoV){
				for each(trait in script_info.traits_infoV){
					switch(trait.kind_trait_type){
						case TraitTypeAndAttributes.Method:
						case TraitTypeAndAttributes.Getter:
						case TraitTypeAndAttributes.Setter:
			...
			if(method_infoV.length){
				...
				for each(var method_info:Method_info in method_infoV){
					i++;
					method_info.methodName=methodNameArr[i];
			...
			if(method_body_infoV.length){
				...
				for each(var method_body_info:Method_body_info in method_body_infoV){
					method_body_info.methodName=method_infoV[method_body_info.method].methodName;
