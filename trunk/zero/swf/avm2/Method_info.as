	...
	public class Method_info{//implements I_zero_swf_CheckCodesRight{
		...
		CONFIG::USE_XML{
			...
			public function toXML(xmlName:String,_toXMLOptions:Object/*zero_swf_ToXMLOptions*/):XML{
				var xml:XML=<{xmlName} class="zero.swf.avm2.Method_info"
				...
				if(param_typeV.length){
					...
					for each(var param_type:int in param_typeV){
						param_typeListXML.appendChild(<param_type
							value={param_type}
							info={param_type>0?_toXMLOptions.multiname_infoV[param_type].toInfo(_toXMLOptions):"(undefined)"}
						/>);
					}
				...
				if(option_detailV){
					if(option_detailV.length){
						var option_detailListXML:XML=<option_detailList count={option_detailV.length}/>
						for each(var option_detail:Option_detail in option_detailV){
							option_detailListXML.appendChild(option_detail.toXML("option_detail",_toXMLOptions));
						}
						xml.appendChild(option_detailListXML);
					}
				}
				if(param_nameV){
					if(param_nameV.length){
						...
						for each(var param_name:int in param_nameV){
							param_nameListXML.appendChild(<param_name
								value={param_name}
								info={param_name>0?_toXMLOptions.stringV[param_name]:"(undefined)"}
							/>);
						}
			...
			public function initByXML(xml:XML,_initByXMLOptions:Object/*zero_swf_InitByXMLOptions*/):void{
				...
				var option_detailXMLList:XMLList=xml.option_detailList.option_detail;
				if(option_detailXMLList.length()){
					i=-1;
					option_detailV=new Vector.<Option_detail>();
