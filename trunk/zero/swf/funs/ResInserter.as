...
package zero.swf.funs{
	...
	public class ResInserter{
		...
		private var swf:SWFExtend;
		private var frameObjs:Object;
		
		
		public function ResInserter(_swf:SWFExtend){
			swf=_swf;
		...
		public function insert(
			...
			var defId:int=swf.getAvalibleDefineObjId();
			
			if(addDoABC){
				frameObj.tagV.push(SimpleDoABC.getDoABCTag(className,type,false));
			}
		...
		public function getTagVAndReset():void{
			...
			var frameId:int=0;
			for each(var swfTag:Tag in swf.tagV){
			...
			if(frameObjs["~-1"]){
				var frameObj:FrameObj=frameObjs["~"+(frameId-1)];//最后一帧的 frameObj
				if(frameObj){
			...
			var swfTagId:int=swf.tagV.length;
			while(--swfTagId>=0){
				swfTag=swf.tagV[swfTagId];
				if(swfTag.type==TagTypes.ShowFrame){
					frameId--;
					...
					if(frameObj){
						if(frameObj.classNameV.length){
							...
							swf.tagV.splice(swfTagId,0,symbolClassTag);
						}
						var tagId:int=frameObj.tagV.length;
						while(--tagId>=0){
							swf.tagV.splice(swfTagId,0,frameObj.tagV[tagId]);
						}
					}
				}
			}
