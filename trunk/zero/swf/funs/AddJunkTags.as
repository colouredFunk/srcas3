...
package zero.swf.funs{
	...
	public class AddJunkTags{
		...
		public static function add(swf:SWF,total:int=10):void{
			...
			///*
			//total个插入到 JunkSprite 中的 loopPlaceObject2
			for each(id in getIdV(total,posV.length)){
				posV[id].tagV.push(
					getJunkSpriteTag(
			...
			///*
			//total个插入到正常 DefineSprite 中的 loopPlaceObject2
			for each(id in getIdV(total,in_normal_sprite_loopPlaceObject2PosV.length)){
			...
			///*
			//total个插入到正常 DefineSprite 中的 loopAction
			for each(id in getIdV(total,in_normal_sprite_posV.length)){
