		...
		public function gen(markStr:String):ABCMultiname{
			...
			if(multiname){
				...
				if(markStr.indexOf("[")==0){
					...
					var id:int=markStr.indexOf("]");
					multiname.kind=MultinameKinds[markStr.substr(1,id-1)];
					if(multiname.kind>0){
						var escapeMarkStr:String=markStr.substr(id+1);
					}else{
				...
				switch(multiname.kind){
					...
					case MultinameKinds.QNameA:
						if(escapeMarkStr.indexOf("(ns=undefined)")==0){
							...
							var dotId:int=escapeMarkStr.lastIndexOf(".");
							if(dotId==-1){
						...
						var execResult:Array=/^([\s\S]*?)(?:\((\d+)\))?$/.exec(escapeMarkStr);
