	...
	public class CodesRunner{
		...
		public function runCodeArr(
			...
			loop:while(i<L){
				...
				/*
				if(code is AVM1Code){
					trace(AVM1Op.opNameV[code.op],code.value);
				}else if(code is AVM1LabelMark){
					trace(code);
				}else{
					trace(AVM1Op.opNameV[code]);
				}
				//*/
				...
				if(code is AVM1LabelMark){
					...
					switch(code){
						case AVM1Op.end://0x00
							break loop;
						break;
						//0x01//nop
						//0x02
						//0x03
						case AVM1Op.nextFrame://0x04
						...
						break;
						case AVM1Op.toggleQuality://0x08
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.stopSounds://0x09
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.oldAdd://0x0a
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.subtract://0x0b
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1-value2);
						break;
						case AVM1Op.multiply://0x0c
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1*value2);
						break;
						case AVM1Op.divide://0x0d
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1/value2);
						break;
						case AVM1Op.oldEquals://0x0e
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.oldLessThan://0x0f
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.and://0x10
						...
						case AVM1Op.or://0x11
							...
							stack.push(value1||value2);
						break;
						case AVM1Op.not://0x12
							stack.push(!stack.pop());
						break;
						case AVM1Op.stringEq://0x13
							//貌似都是用 equals
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1==value2);
						break;
						case AVM1Op.stringLength://0x14
						...
						//0x1a
						//0x1b
						case AVM1Op.getVariable://0x1c
							stack.push(getVar(stack.pop(),globalVars,vars));
						break;
						case AVM1Op.setVariable://0x1d
							value=stack.pop();
							vars[stack.pop()]=value;
						break;
						//0x1e
						//0x1f
						case AVM1Op.setTargetExpr://0x20
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.concat://0x21
							value2=stack.pop();
							value1=stack.pop();
							stack.push(String(value1)+String(value2));
						break;
						case AVM1Op.getProperty://0x22
						...
						case AVM1Op.setProperty://0x23
							...
							obj[varName]=value;
						break;
						case AVM1Op.duplicateClip://0x24
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.removeClip://0x25
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.trace_://0x26
							trace(stack.pop());
						break;
						case AVM1Op.startDrag://0x27
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.stopDrag://0x28
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.stringLess://0x29
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.throw_://0x2a
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.cast://0x2b
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.implements_://0x2c
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.FSCommand2://0x2d
							//暂时没见到...
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						//0x2e
						//0x2f
						case AVM1Op.random://0x30
						...
						break;
						case AVM1Op.getTimer://0x34
							stack.push(getTimer());
						break;
						case AVM1Op.mbSubstring://0x35
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.mbOrd://0x36
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.mbChr://0x37
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						//0x38
						//0x39
						case AVM1Op.delete_://0x3a
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.delete2://0x3b
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.varEquals://0x3c
							//trace("stack="+stack);
							value=stack.pop();
							vars[stack.pop()]=value;
						break;
						case AVM1Op.callFunction://0x3d
							fName=stack.pop();
							argsArr=new Array();
							count=stack.pop();
							while(--count>=0){
						...
						case AVM1Op.return_://0x3e
							...
							return value;
						break;
						case AVM1Op.modulo://0x3f
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1%value2);
						break;
						case AVM1Op.new_://0x40
							fName=stack.pop();
							argsArr=new Array();
							count=stack.pop();
							while(--count>=0){
						...
						case AVM1Op.initObject://0x43
							...
							stack.push(obj);
						break;
						case AVM1Op.typeof_://0x44
							stack.push(typeof(stack.pop()));
						break;
						case AVM1Op.targetPath://0x45
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.enumerate://0x46
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.add://0x47
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1+value2);
						break;
						case AVM1Op.lessThan://0x48
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1<value2);
						break;
						case AVM1Op.equals://0x49
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1==value2);
						break;
						case AVM1Op.toNumber://0x4a
							stack.push(Number(stack.pop()));
						break;
						case AVM1Op.toString_://0x4b
							stack.push(String(stack.pop()));
						break;
						case AVM1Op.dup://0x4c
							value=stack.pop();
							stack.push(value);
							stack.push(value);
						break;
						case AVM1Op.swap://0x4d
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value2);
							stack.push(value1);
						break;
						case AVM1Op.getMember://0x4e
							varName=stack.pop();
							obj=stack.pop();
							stack.push(obj[varName]);
						break;
						case AVM1Op.setMember://0x4f
							value=stack.pop();
							varName=stack.pop();
							obj=stack.pop();
							obj[varName]=value;
						break;
						case AVM1Op.increment://0x50
							stack.push(stack.pop()+1);
						break;
						case AVM1Op.decrement://0x51
							stack.push(stack.pop()-1);
						break;
						case AVM1Op.callMethod://0x52
							fName=stack.pop();
							obj=stack.pop();
							argsArr=new Array();
							count=stack.pop();
						...
						break;
						case AVM1Op.instanceOf://0x54
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1 is value2);
						break;
						case AVM1Op.enumerateValue://0x55
							obj=stack.pop();
							stack.push(null);
							for(varName in obj){
								stack.push(varName);
						...
						//0x5e
						//0x5f
						case AVM1Op.bitwiseAnd://0x60
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1&value2);
						break;
						case AVM1Op.bitwiseOr://0x61
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1|value2);
						break;
						case AVM1Op.bitwiseXor://0x62
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1^value2);
						break;
						case AVM1Op.shiftLeft://0x63
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1<<value2);
						break;
						case AVM1Op.shiftRight://0x64
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1>>value2);
						break;
						case AVM1Op.shiftRight2://0x65
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1>>>value2);
						break;
						case AVM1Op.strictEquals://0x66
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1===value2);
						break;
						case AVM1Op.greaterThan://0x67
							value2=stack.pop();
							value1=stack.pop();
							stack.push(value1>value2);
						break;
						case AVM1Op.stringGreater://0x68
							throw new Error("暂不支持：op="+code+","+AVM1Op.opNameV[code]);
						break;
						case AVM1Op.extends_://0x69
						...
						default:
							if(code is AVM1Code){
								switch(code.op){
									...
									//0x85
									//0x86
									case AVM1Op.setRegister://0x87
										registerArr[code.value]=stack[stack.length-1];//不 pop
									break;
									case AVM1Op.constants://0x88
										//constCode=code;
									break;
									//0x89
									case AVM1Op.ifFrameLoaded://0x8a
										throw new Error("暂不支持：op="+code.op+","+AVM1Op.opNameV[code.op]);
									break;
									case AVM1Op.setTarget://0x8b
										throw new Error("暂不支持：op="+code.op+","+AVM1Op.opNameV[code.op]);
									break;
									case AVM1Op.gotoLabel://0x8c
										if(thisObj.hasOwnProperty("gotoAndStop")){
									...
									break;
									case AVM1Op.ifFrameLoadedExpr://0x8d
										throw new Error("暂不支持：op="+code.op+","+AVM1Op.opNameV[code.op]);
									break;
									case AVM1Op.function2://0x8e
									...
									//0x92
									//0x93
									case AVM1Op.with_://0x94
										throw new Error("暂不支持：op="+code.op+","+AVM1Op.opNameV[code.op]);
									break;
									//0x95
									case AVM1Op.push://0x96
									...
									//0x97
									//0x98
									case AVM1Op.branch://0x99
										i=labelIdDict[code.value];
									break;
									case AVM1Op.getURL2://0x9a
										throw new Error("暂不支持：op="+code.op+","+AVM1Op.opNameV[code.op]);
									break;
									case AVM1Op.function_://0x9b
										if(code.value.FunctionName){
											vars[code.value.FunctionName]=new Fun(codeArr.slice(i,labelIdDict[code.value.endMark]+1));
										}else{
											stack.push(new Fun(codeArr.slice(i,labelIdDict[code.value.endMark]+1)));
									...
									break;
									//0x9c
									case AVM1Op.branchIfTrue://0x9d
										if(stack.pop()){
											i=labelIdDict[code.value];
										}
									break;
									case AVM1Op.callFrame://0x9e
										throw new Error("暂不支持：op="+code.op+","+AVM1Op.opNameV[code.op]);
									break;
									case AVM1Op.gotoFrame2://0x9f
										throw new Error("暂不支持：op="+code.op+","+AVM1Op.opNameV[code.op]);
									break;
									//0xa0
									//0xa1
