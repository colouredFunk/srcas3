...
package zero.swf.funs{
	...
	public class RawDatas2BmdOrLoaderAndDatas{
		...
		public function RawDatas2BmdOrLoaderAndDatas(_rawDatas2BmdOrLoaderAndDatasFinished:Function,...rawDatas){
			...
			for each(var rawData:ByteArray in getRawDataArr(rawDatas)){
				...
				switch(FileTypes.getType(rawData)){
					...
					case FileTypes.BMP:
						...
						dataArr[i]=PNGEncoder.encode(bmdOrLoaderArr[i]);
			...
			imgsSWFLoader.loadBytes(imgDatas2SWFData(imgDataArr,classNameArr),getLoaderContext());
			rest++;
		}
		...
		private function getRawDataArr(rawDatas:*):Array{
			...
			}else if(getQualifiedClassName(rawDatas)=="flash.filesystem::File"){
				rawDataArr.push(getDefinitionByName("zero.air.ReadAndWriteFile").readDataFromFile(rawDatas));
			}else{
		...
		private function loadComplete(event:Event):void{
			...
			*/
			...
			if(--rest<=0){
				imgsSWFLoader=null;
				switch(bmdOrLoaderArr.length){
					case 0:
						throw new Error("未解码任何内容");
					break;
					case 1:
						switch(rawDatas2BmdOrLoaderAndDatasFinished.length){
							//case 0:
							//	throw new Error("参数个数至少为1");
							//break;
							case 1:
								rawDatas2BmdOrLoaderAndDatasFinished(bmdOrLoaderArr[0]);
							break;
							default:
								rawDatas2BmdOrLoaderAndDatasFinished(bmdOrLoaderArr[0],dataArr[0]);
							break;
						}
					break;
					default:
						switch(rawDatas2BmdOrLoaderAndDatasFinished.length){
							...
							//break;
							case 1:
								rawDatas2BmdOrLoaderAndDatasFinished(bmdOrLoaderArr);
							break;
							default:
