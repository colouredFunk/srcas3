...
package zero.getfonts{
	...
	import zero.getfonts.swf.tagBodys.DoABC;
	import zero.getfonts.swf.tagBodys.SymbolClass;
	...
	public class GetFont{
		...
		public static var charDatas:Object;
		
		private static var totalLoaded:int;//总请求数
		
		private static const htmlTxt:TextField=new TextField();//- -
		
		reset();
		
		public static function reset():void{
			for(var fontName0:String in charDatas){
				var i:int=charDatas[fontName0].getFontV.length;
				while(--i>=0){
					var getFont:GetFont=charDatas[fontName0].getFontV[i];
					clear(getFont);
				}
				delete charDatas[fontName0].getFontV;
				delete charDatas[fontName0].shellData;
				delete charDatas[fontName0].groupDatas;
				delete charDatas[fontName0].charArr;
				delete charDatas[fontName0];
			}
			charDatas=new Object();
			totalLoaded=0;
		}
		public static function clear(getFont:GetFont):void{
			var id:int=charDatas[getFont.fontName0].getFontV.indexOf(getFont);
			if(id>-1){
				charDatas[getFont.fontName0].getFontV.splice(id,1);
			}
			getFont.clear();
		}
		...
		public static function initBySWFData(swfData:ByteArray):void{
			...
			for each(var tag:Tag in swf.tagV){
				switch(tag.type){
					...
					case TagTypes.DefineFont3:
						...
						fontObjArr[FontID]={
							getFont:new GetFont(defineFont3.FontName),
						...
						for each(var code:int in defineFont3.CodeV){
							...
							char.code=code;
							charDatas[fontObjArr[FontID].getFont.fontName0].charArr[char.code]=char;//- -
			...
			for each(var fontObj:Object in fontObjArr){
				if(fontObj.charV.length){
					(fontObj.getFont as GetFont).initShellData(fontObj.shellData);
					(fontObj.getFont as GetFont).load("",null);
				}else{
					//trace("GetFont.charDatas[fontObj.getFont.fontName0].getFontV="+GetFont.charDatas[fontObj.getFont.fontName0].getFontV);
					GetFont.clear(fontObj.getFont);
					//trace("GetFont.charDatas[fontObj.getFont.fontName0].getFontV="+GetFont.charDatas[fontObj.getFont.fontName0].getFontV);
				}
			}
		}
		
		public static function canShowText(txt:TextField,text:String):Boolean{
			//判断 txt 是否能显示 text 中的所有字符
			var tf:TextFormat=txt.defaultTextFormat;
			//trace("canShowTexts，tf.font="+tf.font);
			if(tf.font){
				var txtFont:Font=null;
				var enumerateFonts:Array=Font.enumerateFonts(false);
				//trace("enumerateFonts.length="+enumerateFonts.length);
				for each(var font:Font in enumerateFonts){
					if(font.fontName==tf.font){
						txtFont=font;
						break;
					}
				}
				if(txtFont){
					for each(var c:String in getUnrepeatedChars(text).split("")){
						if(txtFont.hasGlyphs(c)){
						}else{
							//trace("字体："+txtFont.fontName+"，不可显示："+text+"（"+c+"）");
							return false;
						}
					}
					//trace("字体："+txtFont.fontName+"，可显示："+text);
					return true;
				}
			}
			//trace("未知字体（tf.font="+tf.font+"，txtFont="+txtFont+"，不可显示："+text);
			return false;
		}
		
		private static var initTxtObjArr:Array=new Array();
		public static function initTxt(
			_txt:TextField,
			textOrXML:*,
			_onInitTxtComplete:Function=null,
			_html:Boolean=false,
			check:Boolean=true
		):void{
			var currInitTxtObj:Object={
				txt:_txt,
				onInitTxtComplete:_onInitTxtComplete,
				html:_html
			}
			initTxtObjArr.push(currInitTxtObj);
			if(textOrXML is XML){
				currInitTxtObj.xml=textOrXML;
			}else{
				currInitTxtObj.xml=<txt text={textOrXML}/>;
			}
			//trace("initTxt：currInitTxtObj.xml.@text.toString()"+currInitTxtObj.xml.@text.toString());
			var text:String=currInitTxtObj.xml.@text.toString();
			if(_html){
			...
			if(
				check
				&&
				canShowText(currInitTxtObj.txt,text)
			){
				initTxtComplete(currInitTxtObj.txt);
			}else{
				var fontName0:String=currInitTxtObj.txt.defaultTextFormat.font.replace(/@getFont\d+$/,"");
				if(charDatas[fontName0]){
					for each(var getFont:GetFont in charDatas[fontName0].getFontV){
						if(getFont.canShowText(text)){
							currInitTxtObj.getFont=getFont;
							//trace("currInitTxtObj.getFont.fontName="+currInitTxtObj.getFont.fontName);
							initTxtComplete(currInitTxtObj.getFont);
							return;
						}
					}
				}
				currInitTxtObj.getFont=new GetFont(fontName0);
				currInitTxtObj.getFont.load(text,initTxtComplete);
			}
		}
		private static function initTxtComplete(getFontOrTxt:*):void{
			
			var currInitTxtObj:Object=null;
			for each(var initTxtObj:Object in initTxtObjArr){
				if(
					getFontOrTxt is TextField
					&&
					initTxtObj.txt==getFontOrTxt
					||
					getFontOrTxt is GetFont
					&&
					initTxtObj.getFont==getFontOrTxt
					
				){
					currInitTxtObj=initTxtObj;
					initTxtObjArr.splice(initTxtObjArr.indexOf(currInitTxtObj),1);
					break;
				}
			}
			if(currInitTxtObj){
			}else{
				trace("initTxtComplete，currInitTxtObj="+currInitTxtObj);
				return;
			}
			
			//trace(currInitTxtObj.txt.defaultTextFormat.font,currInitTxtObj.xml.@text.toString());
			...
			if(getFontOrTxt is GetFont){
				tf.font=(getFontOrTxt as GetFont).fontName;
			}
			for each(var attXML:XML in currInitTxtObj.xml.attributes()){
				switch(attXML.name().toString()){
					...
					break;
					default:
						if(tf.hasOwnProperty(attXML.name().toString())){
							if(attXML.toString()){
								tf[attXML.name().toString()]=int(attXML.toString());
							}
						}
					break;
				}
			...
			if(currInitTxtObj.html){
				currInitTxtObj.txt.htmlText='<font face="'+tf.font+'" color="#'+(0x10000000+tf.color).toString(16).substr(1)+'" size="'+tf.size+'">'+currInitTxtObj.xml.@text.toString().replace(/\r\n/g,"\n")+'</font>';
			}else{
				currInitTxtObj.txt.text=currInitTxtObj.xml.@text.toString().replace(/\r\n/g,"\n");
			}
			//trace("currInitTxtObj.txt.defaultTextFormat.font="+currInitTxtObj.txt.defaultTextFormat.font);
			//trace("currInitTxtObj.txt.text="+currInitTxtObj.txt.text);
			if(currInitTxtObj.onInitTxtComplete){
				switch(currInitTxtObj.onInitTxtComplete.length){
					case 1:
				...
				currInitTxtObj.onInitTxtComplete=null;
				currInitTxtObj.getFont=null;
				currInitTxtObj.txt=null;
				currInitTxtObj.xml=null;
			}
		}
		
		private static function getUnrepeatedChars(_chars:String):String{
			var chars:String="";
			var mark:Object=new Object();
			for each(var char:String in _chars.split("")){
				if(mark[char]){
				}else{
					mark[char]=true;
					chars+=char;
		...
		private var charGroupDataLoader:URLLoader;
		private var charGroupNameArr:Array;
		private var onLoadGroupsComplete:Function;
		
		private var shellDataLoader:URLLoader;
		private var onLoadShellDataComplete:Function;
		private var charDataLoader:URLLoader;
		private var fontSWFLoader:Loader;//实例化 fontSWFData 为一个 swf
		
		private var currId:int;
		
		private var chars:String;
		private var onGetFontComplete:Function;
		private var fontName0:String;
		private var fontName:String;
		private var fontClass:Class;
		private var font:Font;
		
		private var fontSWFData:ByteArray;
		...
		public function GetFont(_fontName0:String){
			fontName0=_fontName0;
			//if(fontsFolderPath){
			//}else{
			//	throw new Error("fontsFolderPath="+fontsFolderPath);
			//}
			//fontsFolderPath=decodeURI(fontsFolderPath.replace(/\\/g,"/"));
			//if(fontsFolderPath.charAt(fontsFolderPath.length-1)=="/"){
			//}else{
			//	fontsFolderPath+="/";
			//}
			fontFolderPath=fontsFolderPath+fontName0.toLowerCase().replace(/[^a-z0-9]/g,"_")+"/";
			if(charDatas[fontName0]){
			}else{
				charDatas[fontName0]={
					getFontV:new Vector.<GetFont>(),
					groupDatas:new Object(),
					charArr:new Array()
				}
			}
			charDatas[fontName0].getFontV.push(this);
		}
		private function clear():void{
			//var id:int=charDatas[fontName0].getFontV.indexOf(this);
			//if(id>-1){
			//	charDatas[fontName0].getFontV.splice(id,1);
			//}
			if(charGroupDataLoader){
				try{
					charGroupDataLoader.close();
				}catch(e:Error){}
				charGroupDataLoader.removeEventListener(Event.COMPLETE,loadCharGroupDataComplete);
				charGroupDataLoader.removeEventListener(IOErrorEvent.IO_ERROR,loadCharGroupDataError);
				charGroupDataLoader.removeEventListener(SecurityErrorEvent.SECURITY_ERROR,loadCharGroupDataError);
				charGroupDataLoader=null;
			}
			charGroupNameArr=null;
			onLoadGroupsComplete=null;
			if(shellDataLoader){
				try{
					shellDataLoader.close();
				}catch(e:Error){}
				shellDataLoader.removeEventListener(Event.COMPLETE,loadShellDataComplete);
				shellDataLoader.removeEventListener(IOErrorEvent.IO_ERROR,loadShellDataError);
				shellDataLoader.removeEventListener(SecurityErrorEvent.SECURITY_ERROR,loadShellDataError);
				shellDataLoader=null;
			}
			onLoadShellDataComplete=null;
			if(charDataLoader){
				try{
					charDataLoader.close();
				}catch(e:Error){}
				charDataLoader.removeEventListener(Event.COMPLETE,loadCharDataComplete);
				charDataLoader.removeEventListener(IOErrorEvent.IO_ERROR,loadCharDataError);
				charDataLoader.removeEventListener(SecurityErrorEvent.SECURITY_ERROR,loadCharDataError);
				charDataLoader=null;
			}
			if(fontSWFLoader){
				try{
		...
		public function loadGroups(_onLoadGroupsComplete:Function,...charGroupNames):void{
			...
			onLoadGroupsComplete=_onLoadGroupsComplete;
			charGroupNameArr=new Array();
			for each(var charGroupName:String in charGroupNames){
				if(charGroups[charGroupName]){
					charGroupNameArr.push(charGroupName);
				}
			}
			
			loadShellData(startLoadGroups);
		}
		private function startLoadGroups():void{
			charGroupDataLoader=new URLLoader();
			charGroupDataLoader.dataFormat=URLLoaderDataFormat.BINARY;
			charGroupDataLoader.addEventListener(Event.COMPLETE,loadCharGroupDataComplete);
			charGroupDataLoader.addEventListener(IOErrorEvent.IO_ERROR,loadCharGroupDataError);
			charGroupDataLoader.addEventListener(SecurityErrorEvent.SECURITY_ERROR,loadCharGroupDataError);
			currId=-1;
			loadGroup();
		}
		private function loadGroup():void{
			while(charDatas[fontName0].groupDatas[charGroupNameArr[++currId]]){}
			if(currId>=charGroupNameArr.length){
				var _onLoadGroupsComplete:Function;
				if(onLoadGroupsComplete==null){
					_onLoadGroupsComplete=null;
				}else{
					_onLoadGroupsComplete=onLoadGroupsComplete;
					onLoadGroupsComplete=null;
				}
				load("",_onLoadGroupsComplete);
				return;
			}
			charGroupDataLoader.load(new URLRequest(fontFolderPath+charGroupNameArr[currId]+".swf"));totalLoaded++;
			trace("加载 "+fontFolderPath+charGroupNameArr[currId]+".swf，totalLoaded="+totalLoaded);
		}
		private function loadCharGroupDataComplete(...args):void{
			charDatas[fontName0].groupDatas[charGroupNameArr[currId]]=charGroupDataLoader.data;
			getCharsFromData(charGroupDataLoader.data);
			loadGroup();
		}
		private function loadCharGroupDataError(...args):void{
			trace("加载 "+fontFolderPath+charGroupNameArr[currId]+".swf 失败");
			loadGroup();
		}
		
		public function load(_chars:String,_onGetFontComplete:Function):void{
			clear();
			chars=getUnrepeatedChars(_chars).replace(/[\r\n]/g,"");
			onGetFontComplete=_onGetFontComplete;
			
			fontSWFLoader=new Loader();
			fontSWFLoader.contentLoaderInfo.addEventListener(Event.COMPLETE,loadFontSWFComplete);
			
			loadShellData(loadCharDatas);
		}
		
		private function loadShellData(_onLoadShellDataComplete:Function):void{
			onLoadShellDataComplete=_onLoadShellDataComplete;
			if(charDatas[fontName0].shellData){
				initShellData(charDatas[fontName0].shellData);
			}else{
				shellDataLoader=new URLLoader();
				shellDataLoader.dataFormat=URLLoaderDataFormat.BINARY;
				shellDataLoader.addEventListener(Event.COMPLETE,loadShellDataComplete);
				shellDataLoader.addEventListener(IOErrorEvent.IO_ERROR,loadShellDataError);
				shellDataLoader.addEventListener(SecurityErrorEvent.SECURITY_ERROR,loadShellDataError);
				shellDataLoader.load(new URLRequest(fontFolderPath+"shell.swf"));totalLoaded++;
				trace("加载："+fontFolderPath+"shell.swf，totalLoaded="+totalLoaded);
			}
		}
		private function loadShellDataComplete(...args):void{
			initShellData(shellDataLoader.data);
		}
		private function initShellData(shellData:ByteArray):void{
			charDatas[fontName0].shellData=shellData;
			if(fontSWF){
			}else{
				...
				for each(tag in fontSWF.tagV){
					switch(tag.type){
						...
						break;
						case TagTypes.DoABC:
							var ABCData:ABCFileWithSimpleConstant_pool=(tag.getBody(DoABC,{ABCDataClass:ABCFileWithSimpleConstant_pool}) as DoABC).ABCData;
							ABCData.stringV[ABCData.stringV.indexOf("MyFont")]=fontName;
							newTagV.push(tag);
							newTagV=newTagV.concat(shellTagV);
						break;
						case TagTypes.SymbolClass:
							var symbolClass:SymbolClass=tag.getBody(SymbolClass,null);
							symbolClass.NameV[0]=fontName;
							newTagV.push(tag);
						break;
					}
					//trace(TagTypes.typeNameV[tag.type]);
				}
				fontSWF.tagV=newTagV;
			}
			
			if(onLoadShellDataComplete==null){
			}else{
				var _onLoadShellDataComplete:Function=onLoadShellDataComplete;
				onLoadShellDataComplete=null;
				_onLoadShellDataComplete();
			}
		}
		private function loadShellDataError(...args):void{
			throw new Error("加载："+fontFolderPath+"shell.swf 失败");
		}
		
		private function loadCharDatas():void{
			charDataLoader=new URLLoader();
			charDataLoader.dataFormat=URLLoaderDataFormat.BINARY;
			charDataLoader.addEventListener(Event.COMPLETE,loadCharDataComplete);
			charDataLoader.addEventListener(IOErrorEvent.IO_ERROR,loadCharDataError);
			charDataLoader.addEventListener(SecurityErrorEvent.SECURITY_ERROR,loadCharDataError);
			currId=-1;
			loadCharData();
		}
		private function getCharsFromData(data:ByteArray):void{
			var offset:int=0;
			...
			while(offset<endOffset){
				...
				charDatas[fontName0].charArr[char.code]=char;
			}
			if(offset==endOffset){
			}else{
		...
		private function loadCharData():void{
			while(charDatas[fontName0].charArr[chars.charCodeAt(++currId)]){}
			if(currId>=chars.length){
				...
				var charId:int=-1;
				for each(var charCode:int in charCodeArr){
					charId++;
					char=charDatas[fontName0].charArr[charCode];
					//trace("char="+String.fromCharCode(charCode)+"，charCode="+char.code);
					if(defineFont3){
				...
				//trace("defineFont3.CodeV.length="+defineFont3.CodeV.length);
				
				fontSWFData=fontSWF.toSWFData(null);
			...
			//charDataLoader.load(new URLRequest(fontFolderPath+chars.charCodeAt(currId)+".swf"));totalLoaded++;
			//trace("加载："+fontFolderPath+chars.charCodeAt(currId)+".swf，totalLoaded="+totalLoaded);
			charDataLoader.load(new URLRequest(fontFolderPath+"block"+(100000+int(chars.charCodeAt(currId)/blockSize)).toString().substr(1)+".swf"));totalLoaded++;
			trace("加载："+fontFolderPath+"block"+(100000+int(chars.charCodeAt(currId)/blockSize)).toString().substr(1)+".swf，totalLoaded="+totalLoaded);
		}
		private function loadCharDataComplete(...args):void{
			getCharsFromData(charDataLoader.data);
			loadCharData();
		}
		private function loadCharDataError(...args):void{
			//trace("加载 "+chars.charAt(currId)+"（"+chars.charCodeAt(currId)+".swf） 失败");
			trace("加载 "+chars.charAt(currId)+"（block"+(100000+int(chars.charCodeAt(currId)/blockSize)).toString().substr(1)+".swf） 失败");
			loadCharData();
		}
		private function loadFontSWFComplete(...args):void{
			...
			font=new fontClass();
			if(onGetFontComplete==null){
			}else{
				switch(onGetFontComplete.length){
					case 1:
						onGetFontComplete(this);
					break;
					default:
						onGetFontComplete();
					break;
