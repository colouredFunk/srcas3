...
package zero.swf.funs{
	...
	import zero.paths.path_FlashPlayer_exe_compress;
	...
	public class SWF2EXE{
		...
		public static function loadPlayer(playerURL:String=null):void{
			playerURL||(playerURL=path_FlashPlayer_exe_compress);
		...
		public static function swf2exe(
			selfURL:String,//用于判断自己是否 exe
			_swfData:ByteArray,
			_swf2exeProgress:Function,
			_swf2exeFinished:Function
		):void{
			if(selfURL.substr(selfURL.length-4).toLowerCase()==".exe"){
				isSelf=true;
			}else{
				isSelf=false;
			}
