...
package zero.swf.ui{
	...
	import zero.html.*;
	import zero.swf.SWF1;
	
	public class SWF1Board extends HTML{
		private var fr:FileReference;
		private var swf:SWF1;
		public function SWF1Board(){
			init(
				<html>
					<html id="modifyBoard" x="10" y="10">
						...
						<label 										y="120" 	text="x:"/>
						<txt	id="xTxt" 					x="20" 	y="120" 	width="60" height="20" type={TextFieldType.INPUT} restrict="0-9.\\-"/>
						<label 							x="90"		y="120" 	text="y:"/>
						<txt	id="yTxt" 					x="110" 	y="120" 	width="60" height="20" type={TextFieldType.INPUT} restrict="0-9.\\-"/>
						
						<label 										y="150" 	text="宽:"/>
						<txt	id="widthTxt" 			x="20" 	y="150" 	width="60" height="20" type={TextFieldType.INPUT} restrict="0-9."/>
						<label 							x="90"		y="150" 	text="高:"/>
						<txt	id="heightTxt" 			x="110" 	y="150" 	width="60" height="20" type={TextFieldType.INPUT} restrict="0-9."/>
						
						<label 										y="180" 	text="帧频:"/>
						<txt	id="FrameRateTxt" 		x="80" 	y="180" 	width="60" height="20" type={TextFieldType.INPUT} restrict="0-9."/>
						<label	id="FrameRateStatusTxt"	x="150"	y="180"	/>
						
						<btn	id="btnModify" 			x="80" 	y="210" 	text="修改" />
					</html>
					<btn id="btnBrowse" x="180" y="220" text="点击选择SWF" />
					<txt id="outputTxt" x="10" y="250" width="280" height="40"/>
				</html>,
				click,change
			);
		}
		...
		private function click(btn:Btn):void{
			...
			switch(btn.id){
				...
				case "btnModify":
					...
					swf.x=Number(elements["xTxt"].text);
					swf.y=Number(elements["yTxt"].text);
					swf.width=Number(elements["widthTxt"].text);
					swf.height=Number(elements["heightTxt"].text);
					swf.FrameRate=Number(elements["FrameRateTxt"].text);
		...
		private function complete(event:Event):void{
			...
			elements["xTxt"].text=swf.x.toString();
			elements["yTxt"].text=swf.y.toString();
			elements["widthTxt"].text=swf.width.toString();
			elements["heightTxt"].text=swf.height.toString();
			
			elements["FrameRateTxt"].text=swf.FrameRate.toString();
		...
		private function updateStatuses():void{
			...
			if(elements["FrameRateTxt"].text){
				if(int(elements["FrameRateTxt"].text)>0){
					elements["FrameRateStatusTxt"].text="fps";
				}else{
					elements["FrameRateStatusTxt"].text="帧频不能为 0";
					canModify=false;
				}
			}else{
				elements["FrameRateStatusTxt"].text="帧频不能为空";
				canModify=false;
			}
