	...
	public class ACTIONRECORDs{//implements I_zero_swf_CheckCodesRight{
		...
		public function initByData(data:ByteArray,offset:int,endOffset:int,_initByDataOptions:Object/*zero_swf_InitByDataOptions*/):int{
			...
			if(_initByDataOptions){
				if(_initByDataOptions.ActionsGetHexArr){
					var hexByPosArr:Array=new Array();
				}
				if(_initByDataOptions.removeAVM1JunkCodes){//20110905
			...
			var minOffset:int=int.MAX_VALUE;
			var startOffset:int=offset;
			
			var seed:Array=[offset];
			
			while(seed.length){
				var newSeed:Array=new Array();
				for each(var _offset:int in seed){
					if(mark){
						if(mark[_offset]){//已遍历过的 code
							continue;
						}
					}
					offset=_offset;
					...
					var useNextSubSeed:Boolean=false;
					
					//while(true){
					while(offset<endOffset){//貌似可往前跳出 ACTIONRECORDs 的范围，但不可往后跳出
						if(mark){
							mark[offset]=true;
						}else{
							if(offset<endOffset){
						...
						if(op<0x80){
							...
							switch(op){
								...
								case AVM1Ops.getURL://0x83	//STRING,STRING
									...
									if(offset+get_str_size<endOffset){
									}else{
										get_str_size=endOffset-offset;
									}
									data.position=offset;
									code_value={
								...
								case AVM1Ops.push://0x96	//(UI8,STRING|FLOAT|UI8|DOUBLE|UI32|UI16)s
									...
									while(offset<pos+3+Length){
										...
										switch(data[offset++]){
											...
											case 2:
												pushValueV[i]=null;
												//trace("2 pushValueV[i]="+pushValueV[i]);
											break;
											case 3:
												pushValueV[i]=undefined;
												//trace("3 pushValueV[i]="+pushValueV[i]);
											break;
											case 4:
								...
								default:
									...
									///
									Length=data[offset++]|(data[offset++]<<8);
									if(pos+3+Length>data.length){//20110905
										Length=data.length-3-pos;
									}
									
									///
									codeByPosArr[pos]=new ByteArray();
									codeByPosArr[pos].writeBytes(data,pos,3+Length);
									output("对未知代码使用 ByteArray 进行记录："+BytesAndStr16.bytes2str16(codeByPosArr[pos],0,codeByPosArr[pos].length),"brown");
									outputError("未知 op: "+op);
									offset+=Length;
			...
			if(minOffset<startOffset){
				if(labelByPosArr[startOffset]){
				}else{
					labelByPosArr[startOffset]=new LabelMark(++labelId);
				}
				codeId++;
				codeArr[codeId]=new Code(AVM1Ops.jump,labelByPosArr[startOffset]);
			}
			
			for(offset=minOffset;offset<=endOffset;offset++){
		...
		public function toData(_toDataOptions:Object/*zero_swf_ToDataOptions*/):ByteArray{
			...
			if(constCode){
				...
				constStrV=constCode.value;
				if(constStrV.length){
					constStrV.sort(sortConstStr);
					codeArr.unshift(constCode);
					insertConstCodeId=0;
				}
			}
			if(constStrV.length){
...
//								//20111208
//								if(code.value){
//									for each(var c:String in code.value.split("")){
...
//								//20111208
//								if(code.value.UrlString){
//									for each(c in code.value.UrlString.split("")){
//										if(c.charCodeAt(0)>0xff){
//											data.writeUTFBytes(c);
//										}else{
//											data.writeByte(c.charCodeAt(0));
...
//								//20111208
//								if(code.value.TargetString){
//									for each(c in code.value.TargetString.split("")){
//										if(c.charCodeAt(0)>0xff){
//											data.writeUTFBytes(c);
//										}else{
//											data.writeByte(c.charCodeAt(0));
...
//								data.writeByte(0x00);
							...
							case AVM1Ops.constantPool://0x88	//UI16,STRING[Count]
								...
								*/
								...
								///
								var Count:int=code.value.length;
								data[offset++]=Count;
								data[offset++]=Count>>8;
								data.position=offset;
								for each(pushValue in code.value){
...
//									//20111208
//									if(code.value.CatchName){
//										for each(c in code.value.CatchName.split("")){
//											if(c.charCodeAt(0)>0xff){
//												data.writeUTFBytes(c);
//											}else{
//												data.writeByte(c.charCodeAt(0));
...
//									for each(c in code.value.FunctionName.split("")){
//										if(c.charCodeAt(0)>0xff){
//											data.writeUTFBytes(c);
//										}else{
//											data.writeByte(c.charCodeAt(0));
...
//										for each(c in Parameter.ParamName.split("")){
//											if(c.charCodeAt(0)>0xff){
//												data.writeUTFBytes(c);
//											}else{
//												data.writeByte(c.charCodeAt(0));
...
//									data.writeByte(0x00);
			...
			if(insertConstCodeId>-1){
				codeArr.splice(insertConstCodeId,1);
			}
			
			return data;
		}
		private function sortConstStr(str1:String,str2:String):int{
			var str1_lower:String=str1.toLowerCase();
			var str2_lower:String=str2.toLowerCase();
			...
			if(codeArr.length){
				...
				for each(var code:* in codeArr){
					...
					if(code is LabelMark){
						...
						output("使用 Array 进行记录的未知代码："+code,"brown");
						var numStr:String="";
						for each(var num:int in code){
						...
						if(code is int){
							var opName:String=AVM1Ops.opNameV[code];//20110906
							if(opName){
								codesStr+="\t\t\t\t\t"+opName+"\n";
							}else{
								codesStr+="\t\t\t\t\t"+BytesAndStr16._16V[code]+"\n";
							}
						}else if(code is Code){
							switch(code.op){
								case AVM1Ops.call://0x9e	//
							...
							switch(op){
								...
								case AVM1Ops.if_://0x9d		//LabelMark
									...
									if(code_value){
										...
										output("找不到对应的 labelMark: "+codeStr,"brown");
										codeArr[codeId]=[op,0x02,0x00,0x00,0x00];
									}
								break;
