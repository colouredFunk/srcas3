	...
	public class Multiname_info{//implements I_zero_swf_CheckCodesRight{
		...
		CONFIG::USE_XML{
			public function toXML(xmlName:String,_toXMLOptions:Object/*zero_swf_ToXMLOptions*/):XML{
				var xml:XML=<{xmlName} class="zero.swf.avm2.Multiname_info"
				...
				switch(kind){
					...
					case MultinameKinds.QNameA:
						xml.@u30_1=u30_1;
						xml.@u30_2=u30_2;
						xml.@info=_toXMLOptions.namespace_infoV[u30_1].toInfo(_toXMLOptions)+"."+_toXMLOptions.stringV[u30_2];
					break;
					case MultinameKinds.Multiname:
					case MultinameKinds.MultinameA:
						xml.@u30_1=u30_1;
						xml.@u30_2=u30_2;
						xml.@info=_toXMLOptions.ns_set_infoV[u30_2].toInfo(_toXMLOptions)+"."+_toXMLOptions.stringV[u30_1];
					break;
					case MultinameKinds.RTQName:
					case MultinameKinds.RTQNameA:
						xml.@u30_1=u30_1;
						xml.@info=_toXMLOptions.stringV[u30_2];
					break;
					case MultinameKinds.RTQNameL:
					case MultinameKinds.RTQNameLA:
						//
					...
					case MultinameKinds.MultinameLA:
						xml.@u30_1=u30_1;
						xml.@info=_toXMLOptions.ns_set_infoV[u30_1].toInfo(_toXMLOptions);
					break;
					case MultinameKinds.GenericName:
						xml.@u30_1=u30_1;
						xml.@info=_toXMLOptions.multiname_infoV[u30_1].toInfo(_toXMLOptions);
						if(ParamV.length){
							var ParamListXML:XML=<ParamList count={ParamV.length}/>
							for each(var Param:int in ParamV){
								ParamListXML.appendChild(<Param
									value={Param}
									info={_toXMLOptions.multiname_infoV[Param].toInfo(_toXMLOptions)}
								/>);
							}
			...
			public function toInfo(_toXMLOptions:Object):String{
				switch(kind){
					case MultinameKinds.QName:
					case MultinameKinds.QNameA:
						return "["+MultinameKinds.kindV[kind]+"]"+_toXMLOptions.namespace_infoV[u30_1].toInfo(_toXMLOptions)+"."+_toXMLOptions.stringV[u30_2];
					break;
					case MultinameKinds.Multiname:
					case MultinameKinds.MultinameA:
						return "["+MultinameKinds.kindV[kind]+"]"+_toXMLOptions.ns_set_infoV[u30_2].toInfo(_toXMLOptions)+"."+_toXMLOptions.stringV[u30_1];
					break;
					case MultinameKinds.RTQName:
					case MultinameKinds.RTQNameA:
						return "["+MultinameKinds.kindV[kind]+"]"+_toXMLOptions.stringV[u30_2];
					break;
					case MultinameKinds.RTQNameL:
					case MultinameKinds.RTQNameLA:
						return "["+MultinameKinds.kindV[kind]+"]";
					break;
					case MultinameKinds.MultinameL:
					case MultinameKinds.MultinameLA:
						return "["+MultinameKinds.kindV[kind]+"]"+_toXMLOptions.ns_set_infoV[u30_1].toInfo(_toXMLOptions);
					break;
					case MultinameKinds.GenericName:
						...
						if(_toXMLOptions.markGenericNameDict){//20120221
						}else{
							_toXMLOptions.markGenericNameDict=new Dictionary();
						}
						if(_toXMLOptions.markGenericNameDict[this]){
							outputError("重复使用的 Vector");
							return "(重复使用的 Vector)";
						}
						_toXMLOptions.markGenericNameDict[this]=true;
						
						var info:String="";
						for each(var Param:int in ParamV){
							info+=","+_toXMLOptions.multiname_infoV[Param].toInfo(_toXMLOptions);
						}
						return "["+MultinameKinds.kindV[kind]+"]"+_toXMLOptions.multiname_infoV[u30_1].toInfo(_toXMLOptions)+".<"+info.substr(1)+">";
			...
			public function initByXML(xml:XML,_initByXMLOptions:Object/*zero_swf_InitByXMLOptions*/):void{
				...
				switch(kind){
					...
					case MultinameKinds.GenericName:
						...
						var i:int=-1;
						ParamV=new Vector.<int>();
						for each(var ParamXML:XML in xml.ParamList.Param){
							i++;
