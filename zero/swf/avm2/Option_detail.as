...
创建时间:2010年10月18日 23:21:31 (代码生成器: F:/airs/program files2/CodesGenerater/bin-debug/CodesGenerater.swf) 
历次修改:未有修改
用法举例:这家伙很懒什么都没写
*/
//option_detail
		...
		override public function initByData(data:ByteArray,offset:int,endOffset:int):int{
			...
			if(data[offset]>>>7){
				if(data[offset+1]>>>7){
					if(data[offset+2]>>>7){
						if(data[offset+3]>>>7){
							val=(data[offset++]&0x7f)|((data[offset++]&0x7f)<<7)|((data[offset++]&0x7f)<<14)|((data[offset++]&0x7f)<<21)|(data[offset++]<<28);
						}else{
							val=(data[offset++]&0x7f)|((data[offset++]&0x7f)<<7)|((data[offset++]&0x7f)<<14)|(data[offset++]<<21);
						}
					}else{
						val=(data[offset++]&0x7f)|((data[offset++]&0x7f)<<7)|(data[offset++]<<14);
					}
				}else{
					val=(data[offset++]&0x7f)|(data[offset++]<<7);
				}
			}else{
				val=data[offset++];
			}
		...
		override public function toData():ByteArray{
			...
			if(val>>>7){
				if(val>>>14){
					if(val>>>21){
						if(val>>>28){
							data[offset++]=(val&0x7f)|0x80;
							data[offset++]=((val>>>7)&0x7f)|0x80;
							data[offset++]=((val>>>14)&0x7f)|0x80;
							data[offset++]=((val>>>21)&0x7f)|0x80;
							data[offset++]=val>>>28;
						}else{
							data[offset++]=(val&0x7f)|0x80;
							data[offset++]=((val>>>7)&0x7f)|0x80;
