...
创建时间:2010年10月19日 19:59:47 (代码生成器: F:/airs/program files2/CodesGenerater/bin-debug/CodesGenerater.swf) 
历次修改:未有修改
用法举例:这家伙很懒什么都没写
*/
		...
		override public function initByData(data:ByteArray,offset:int,endOffset:int):int{
			...
			if(data[offset]>>>7){
				if(data[offset+1]>>>7){
					if(data[offset+2]>>>7){
						if(data[offset+3]>>>7){
							init=(data[offset++]&0x7f)|((data[offset++]&0x7f)<<7)|((data[offset++]&0x7f)<<14)|((data[offset++]&0x7f)<<21)|(data[offset++]<<28);
						}else{
							init=(data[offset++]&0x7f)|((data[offset++]&0x7f)<<7)|((data[offset++]&0x7f)<<14)|(data[offset++]<<21);
						}
					}else{
						init=(data[offset++]&0x7f)|((data[offset++]&0x7f)<<7)|(data[offset++]<<14);
					}
				}else{
					init=(data[offset++]&0x7f)|(data[offset++]<<7);
				}
			}else{
				init=data[offset++];
			}
		...
		override public function toData():ByteArray{
			...
			if(init>>>7){
				if(init>>>14){
					if(init>>>21){
						if(init>>>28){
							data[offset++]=(init&0x7f)|0x80;
							data[offset++]=((init>>>7)&0x7f)|0x80;
							data[offset++]=((init>>>14)&0x7f)|0x80;
							data[offset++]=((init>>>21)&0x7f)|0x80;
							data[offset++]=init>>>28;
						}else{
							data[offset++]=(init&0x7f)|0x80;
							data[offset++]=((init>>>7)&0x7f)|0x80;
