...
我补的
			...
			var opNameArr:Array=[
				...
				["u8_s24_u30_s24List__lookupswitch",["lookupswitch"]],
				
				["u8_u8_u30_u8_u30__debug",["debug"]]
			];
			
			var subOpNameArr:Array;
			var opDataTypeMark:Object=new Object();
			var dataTypeMark:Object=new Object();
			var opTypeMark:Object=new Object();
			var dataType_:String;
			var type_:String;
			for each(subOpNameArr in opNameArr){
				type_=subOpNameArr[0];
				dataType_=type_.split("__")[0];
				if(dataTypeMark[dataType_]){
				}else{
					dataTypeMark[dataType_]=true;
					codes+="\t\t\n\t\tpublic static const dataType_"+dataType_+":String=\""+dataType_+"\";\n";
				}
				codes+="\t\tpublic static const type_"+type_+":String=\""+type_+"\";\n";
				for each(opName in subOpNameArr[1]){
					opDataTypeMark[opName]=dataType_;
					opTypeMark[opName]=type_;
				}
			}
			codes+="\n";
			...
			codes+="\n";
			codes+="\t\tpublic static var opDataTypeV:Vector.<String>;\n";
			codes+="\t\tpublic static var opTypeV:Vector.<String>;\n";
			codes+="\t\tpublic static var ops:Object;\n";
			codes+="\t\tpublic static const opNameV:Vector.<String>=get_opNameV();\n";
			codes+="\t\tprivate static function get_opNameV():Vector.<String>{\n";
			codes+="\t\t\topDataTypeV=new Vector.<String>(256);\n";
			codes+="\t\t\topDataTypeV.fixed=true;\n";
			codes+="\t\t\topTypeV=new Vector.<String>(256);\n";
			codes+="\t\t\topTypeV.fixed=true;\n";
			codes+="\t\t\tops=new Object();\n";
			...
			for each(avm2Op in avm2OpV){
				...
				dataType_=opDataTypeMark[opName];
				if(dataType_){
				}else{
					throw new Error("dataType_="+dataType_);
				}
				type_=opTypeMark[opName];
				if(type_){
				}else{
					throw new Error("type_="+type_);
				}
				codes+="\t\t\topDataTypeV["+opName+"]=dataType_"+dataType_+";\n";
				codes+="\t\t\topTypeV["+opName+"]=type_"+type_+";\n";
				codes+="\t\t\tops[\""+opName+"\"]="+opName+";\n";
