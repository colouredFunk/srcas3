...
package zero.swf.tagBodys{
	...
	public class DoInitAction{//implements I_zero_swf_CheckCodesRight{
		...
		//public var ActionEndFlag:int;//经测试这是会被播放器执行的（把 ActionEndFlag 设置为 8（toggleQuality）然后打开SWF（不是在实例化类时执行，而是播放器加载到此 tag 后马上执行），将发现播放质量发生变化，所以 ActionEndFlag 应是 Actions 的一部分）
		//
