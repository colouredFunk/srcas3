...
创建时间:2010年12月12日 19:50:56 (代码生成器: F:/airs/program files2/CodesGenerater/bin-debug/CodesGenerater.swf) 
历次修改:未有修改
用法举例:这家伙很懒什么都没写
*/
...
package zero.swf.records{
	import zero.swf.records.SOUNDENVELOPE;
		...
		public function initByData(data:ByteArray,offset:int,endOffset:int):int{
			...
			if(HasEnvelope){
				...
				for(var i:int=0;i<EnvPoints;i++){
					...
					offset=EnvelopeRecordV[i].initByData(data,offset,endOffset);
				}
			}
			return offset;
		}
		public function toData():ByteArray{
			...
			if(HasEnvelope){
				...
				for each(var EnvelopeRecord:SOUNDENVELOPE in EnvelopeRecordV){
					data.writeBytes(EnvelopeRecord.toData());
				}
			offset=data.length;
			}
			return data;
		...
		public function toXML(xmlName:String):XML{
			...
			if(HasEnvelope){
				if(EnvelopeRecordV.length){
					...
					for each(var EnvelopeRecord:SOUNDENVELOPE in EnvelopeRecordV){
						listXML.appendChild(EnvelopeRecord.toXML("EnvelopeRecord"));
					}
			...
			if(HasEnvelope){
				if(xml.EnvelopeRecordList.length()){
					...
					for each(var EnvelopeRecordXML:XML in EnvelopeRecordXMLList){
						i++;
						EnvelopeRecordV[i]=new SOUNDENVELOPE();
						EnvelopeRecordV[i].initByXML(EnvelopeRecordXML);
					}
				}else{
