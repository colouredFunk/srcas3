...
package zero.swf{
	...
	public class SWF{
		...
		public function swfData2Bytes(swfData:ByteArray):ByteArray{
			...
			switch(type){
				...
				case "ZWS"://20120517
					...
					//hasLZMA=false;trace("测试");
					
					//trace("hasLZMA="+hasLZMA);
					...
					if(hasLZMA){
						...
						switch((swfData[15]<<8)|swfData[16]){
							case 0x0001://5d 00 00 00 01 //flash cs6 导出的 swf
								uncompressed_size-=8;
							break;
							case 0x8000://5d 00 00 00 08 //unity 导出的 swf 20130618
								uncompressed_size-=8;
							break;
							default://5d 00 00 10 00 //用 ByteArray.compress("lzma") 生成的 swf
							//case 0x1000:
		 ...
		 * 
		 */		
		public function toFWSData(_toDataOptions:Object):ByteArray{
			...
			for each(var tag:Tag in tagV){
				var tagData:ByteArray=tag.toData(_toDataOptions);
				//trace("tagData.length="+tagData.length);
				bytes.writeBytes(tagData);
			}
			...
			bytes[4]=FileLength;
			bytes[5]=FileLength>>8;
			bytes[6]=FileLength>>16;
			bytes[7]=FileLength>>24;
		...
		/**
		 * 
		 * 直接用 FileMode.APPEND 方式写入每块数据，可能防止大文件编码时内存不足
		 * 
		 */
		public function toFWSDataBlocks(_toDataOptions:Object):Vector.<ByteArray>{
			
			_toDataOptions=toSWFData_start(_toDataOptions);
			
			var infoBytes:ByteArray=infoToBytes(_toDataOptions);
			infoBytes.position=0;
			infoBytes.writeUTFBytes("FWS");
			
			infoBytes[3]=Version;
			
			var FileLength:int=infoBytes.length;
			
			var blockDataV:Vector.<ByteArray>=new <ByteArray>[infoBytes];
			for each(var tag:Tag in tagV){
				var tagData:ByteArray=tag.toData(_toDataOptions);
				FileLength+=tagData.length;
				blockDataV.push(tagData);
			}
			
			infoBytes[4]=FileLength;
			infoBytes[5]=FileLength>>8;
			infoBytes[6]=FileLength>>16;
			infoBytes[7]=FileLength>>24;
