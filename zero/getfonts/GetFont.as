...
package zero.getfonts{
	...
	public class GetFont{
		...
		public static const fontsFolderPath:String="http://www.wanmei.com/public/swf/fonts/";
		...
		private static const charDatas:Object=new Object();
		
		private static var totalLoaded:int=0;//总请求数
		...
		public static function canShowText(txt:TextField,text:String):Boolean{
			...
			if(tf.font){
				...
				if(txtFont){
					for each(var c:String in text.split("")){
		...
		public static function initTxt(
			...
			if(canShowText(currInitTxtObj.txt,currInitTxtObj.xml.@text.toString())){
				initTxtComplete(currInitTxtObj.txt);
			}else{
				currInitTxtObj.getFont=new GetFont(currInitTxtObj.txt.defaultTextFormat.font);
				currInitTxtObj.getFont.load(currInitTxtObj.xml.@text.toString(),initTxtComplete);
			}
		}
		private static function initTxtComplete(getFontOrTxt:*):void{
			...
			if(currInitTxtObj){
				...
				throw new Error("currInitTxtObj="+currInitTxtObj);
			}
			...
			currInitTxtObj.txt.text=currInitTxtObj.xml.@text.toString();
		...
		public function load(_chars:String,_onGetFontComplete:Function):void{
			clear();
			var charArr:Array=_chars.split("");
			chars="";
			var mark:Object=new Object();
			for each(var char:String in charArr){
				if(mark[char]){
				}else{
					mark[char]=true;
					chars+=char;
		...
		private function loadShellDataComplete(...args):void{
			if(fontSWF){
				...
				Tags.getTagsByData(shellTagV,shellDataLoader.data,0,shellDataLoader.data.length);
				...
				fontName=(defineFont3.FontName+=" "+new Date().time);
