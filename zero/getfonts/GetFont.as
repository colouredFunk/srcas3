...
package zero.getfonts{
	...
	public class GetFont{
		...
		public var fontSWFData:ByteArray;
		private var fontSWF:SWF;
		private var defineFont3:DefineFont3;
		private var defineFontAlignZones:DefineFontAlignZones;
		
		public function GetFont(_fontFolderPath:String){
			fontFolderPath=decodeURI(_fontFolderPath.replace(/\\/g,"/"));
			if(fontFolderPath.charAt(fontFolderPath.length-1)=="/"){
			}else{
				fontFolderPath+="/";
			}
			if(charDatas[fontFolderPath]){
			}else{
				charDatas[fontFolderPath]={
		...
		private function loadGroup():void{
			while(charDatas[fontFolderPath].groupDatas[charGroupNameArr[++currId]]){}
			if(currId>=charGroupNameArr.length){
				if(onLoadGroupsComplete==null){
				}else{
					var _onLoadGroupsComplete:Function=onLoadGroupsComplete;
					onLoadGroupsComplete=null;
					_onLoadGroupsComplete();
				}
				return;
			}
			charGroupDataLoader.load(new URLRequest(fontFolderPath+charGroupNameArr[currId]+".swf"));
			trace("加载 "+fontFolderPath+charGroupNameArr[currId]+".swf");
		}
		private function loadCharGroupDataComplete(...args):void{
			charDatas[fontFolderPath].groupDatas[charGroupNameArr[currId]]=charGroupDataLoader.data;
		...
		private function loadShellData(_onLoadShellDataComplete:Function):void{
			...
			if(fontSWF){
				...
				shellDataLoader.load(new URLRequest(fontFolderPath+"shell.swf"));
				trace("加载："+fontFolderPath+"shell.swf");
			}
		}
		...
		private function getCharsFromData(data:ByteArray):void{
			...
			while(offset<endOffset){
				...
				charDatas[fontFolderPath].charArr[char.code]=char;
			}
			if(offset==endOffset){
			}else{
		...
		private function loadCharData():void{
			while(charDatas[fontFolderPath].charArr[chars.charCodeAt(++currId)]){}
			if(currId>=chars.length){
				
				var charCodeArr:Array=new Array();
				for each(var char:Char in charDatas[fontFolderPath].charArr){
				...
				for each(var charCode:int in charCodeArr){
					charId++;
					char=charDatas[fontFolderPath].charArr[charCode];
			...
			charDataLoader.load(new URLRequest(fontFolderPath+chars.charCodeAt(currId)+".swf"));
			trace("加载："+fontFolderPath+chars.charCodeAt(currId)+".swf");
		}
		...
		public function initTxt(txt:TextField):void{
			if(font){
				txt.defaultTextFormat=new TextFormat(font.fontName);
				txt.embedFonts=true;
			}else{
				throw new Error("font="+font+"，请先调用 load() 加载字体");
			}
		}
	}
}