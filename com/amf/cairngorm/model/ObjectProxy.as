package com.amf.cairngorm.model
{
	...
	/**
	  * ModelLocator:实现object和任何对象的绑定,通过代理之后的对象可以和任何对象绑定
	  */
	public dynamic class ObjectProxy extends Proxy
	{
		...
		public function unBind (o:Object, pro:String, t:String = null) 
		{
			...
			for (var i=0; i < _bindings.length; i++) {
				if (_bindings[i].o == o && _bindings[i].pro == pro && _bindings[i].t == t) {
		...
		public function unBindFun(fun:Function, t:String)
		{
			for (var i:int = 0; i < _bindingFuns.length; i++)
			{
				if (_bindingFuns[i].t == t && _bindingFuns[i].fun==fun)
				{
					return _bindingFuns.splice(i, 1);
				}
			}
			return null;
		}
